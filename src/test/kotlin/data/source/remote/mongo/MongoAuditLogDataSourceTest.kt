...
class MongoAuditLogDataSourceTest {
    ...
    @Test
    fun `getEntityLogs should throw GetItemsFailedException when try to get projects fails in MongoDB`() =
        runTest {
            coEvery { mongoClientCollection.find(filter = any()) } throws Exception()

            assertThrows<GetItemsFailedException> {
                remoteAuditLogDataSource.getEntityLogs(
                    "1",
                    AuditLogEntityType.PROJECT,
                )
            }
        }
    ...
    @Test
    fun `saveAuditLog should throw CreationItemFailedException when create audit log fails in MongoDB`() =
        runTest {
    ...
    @Test
    fun `getEntityLogByLogId should throw GetItemByIdFailedException when get audit log by ID fails in MongoDB`() =
        runTest {
            coEvery { mongoClientCollection.find(filter = any()) } throws Exception()
    ...
    @Test
    fun `deleteAuditLog should throw DeleteItemFailedException when delete audit log fails in MongoDB`() =
        runTest {
